...
import { createModifierSchema } from "@/schemas/modifiers";
import { and, eq, ilike, or, asc, desc, count } from "drizzle-orm";

// GET /api/modifiers - Listar modifiers con paginación y filtros
export async function GET(request: NextRequest) {
  try {
    ...
    const conditions = [eq(modifiers.org_id, orgId)];

    if (search) {
      conditions.push(or(ilike(modifiers.name, `%${search}%`))!);
    }
    ...
    switch (finalSortBy) {
      case "name":
        orderColumn = modifiers.name;
        break;
      case "price":
        orderColumn = modifiers.price;
        break;
      case "weight":
        orderColumn = modifiers.weight;
        break;
      case "created_at":
      default:
        orderColumn = modifiers.created_at;
        break;
    }
    const orderDirection = finalSortOrder === "asc" ? asc : desc;

    // Ejecutar consultas en paralelo
    const [modifiersList, totalCountResult] = await Promise.all([
      db
        .select()
        .from(modifiers)
...
// POST /api/modifiers - Crear un nuevo modifier
export async function POST(request: NextRequest) {
  try {
    ...
    const parseResult = createModifierSchema.safeParse(body);
    ...
    return NextResponse.json(
      { modifier: newModifier[0], message: "Modifier creado exitosamente" },
      { status: 201 }
    );
  } catch (error) {
